...
def test_create_and_list_children():
    c = _client()
    r = c.post("/v1/children", json={"name": "Ana", "grade_level": "4º ano"})
    assert r.status_code == 201, r.text
    cid = r.json()["id"]
    lst = c.get("/v1/children").json()
...
def test_schedules_reject_too_short():
    c = _client()
    cid = c.post("/v1/children", json={"name": "Beto"}).json()["id"]
...
def test_activities_and_agenda():
    ...
    ag = c.get(f"/v1/children/{cid}/agenda")
...
def test_suggestions_respect_real_routine():
    ...
    with patch("app.services.vision_openrouter.extract_homework", return_value=ok):
        hid = c.post("/v1/homeworks/upload",
            ...
            data={"child_id": cid}).json()["homework_id"]
    free = c.get("/v1/suggestions", params={"homework_id": hid, "limit": 5}).json()["suggestions"]
    assert len(free) > 0
    ...
    blocked = c.get("/v1/suggestions", params={"homework_id": hid, "limit": 5}).json()["suggestions"]
